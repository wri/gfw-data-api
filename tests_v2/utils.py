...
async def _create_vector_source_assets(dataset_name, version_name):
    ...
    async with ContextEngine("WRITE"):
        await db.all(
            f"""CREATE TABLE "{dataset_name}"."{version_name}" (fid integer, version varchar, geom geometry);"""
        )
        await db.all(
            f"""INSERT INTO "{dataset_name}"."{version_name}" (fid, version, geom) SELECT 1, '{version_name}', ST_GeomFromGeoJSON('{json.dumps(geojson["features"][0]["geometry"])}');"""
        )


async def _create_vector_revision_assets(dataset_name, version_name, source_version):
    from app.application import db
    ...
    async with ContextEngine("WRITE"):
        await db.all(
            f"""INSERT INTO "{dataset_name}"."{source_version}" (fid, version, geom) SELECT 1, '{version_name}', ST_GeomFromGeoJSON('{json.dumps(geojson["features"][0]["geometry"])}');"""
        )
