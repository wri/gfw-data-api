...
from app.utils.google import get_matching_gs_files
...
async def test_get_matching_gs_files_no_filtering(monkeypatch: MonkeyPatch):
    ...
    keys = await get_matching_gs_files(good_bucket, good_prefix)
    assert len(keys) == 3
...
async def test_get_matching_gs_files_match_extensions(monkeypatch: MonkeyPatch):
    ...
    keys = await get_matching_gs_files(good_bucket, good_prefix, extensions=[".tif"])
...
async def test_get_matching_gs_files_no_matches(monkeypatch: MonkeyPatch):
    ...
    keys = await get_matching_gs_files(good_bucket, good_prefix, extensions=[".pdf"])
    assert keys == []
...
async def test_get_matching_gs_files_early_exit(monkeypatch: MonkeyPatch):
    ...
    keys = await get_matching_gs_files(good_bucket, good_prefix, exit_after_max=1)
    assert len(keys) == 1
