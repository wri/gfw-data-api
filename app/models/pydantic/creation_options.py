...
class ZoomRangeWhereFilter(StrictBaseModel):
    min_zoom: int
    max_zoom: int
    field: str
    values_in: List[StrictInt | str]
...
class StaticVectorTileCacheCreationOptions(TileCacheBaseModel):
    ...
    where_filters: Optional[List[ZoomRangeWhereFilter]] = Field(
        None,
        description="Optional per-zoom-range filters to apply to features."
    )
