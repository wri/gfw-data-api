...
def upgrade():
    ...
    op.get_bind().execute(text(
        """UPDATE assets
                SET creation_options = jsonb_set(creation_options, '{indices}', REGEXP_REPLACE((creation_options->'indices')::text, :regex, :replacement, :scope)::jsonb, false)
                WHERE (asset_type = 'Geo database table' or asset_type = 'Database table') AND creation_options ? 'indices';
        """), **{
            "regex": '"column_name": ("[_[:alpha:]]*")',
            "replacement": '"column_names": [\1]',
            "scope": 'g'}
    )
