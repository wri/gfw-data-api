...
def get_metadata():
            ...
            **VersionMetadata(
                **dict(
                    chain(
                        ...
                        {
                            "content_date_range": {
                                "start_date": version[2]
                                .get("content_date_range", {})
                                .get("min"),
                                "end_date": version[2]
                                .get("content_date_range", {})
                                .get("max"),
    ...
    processed_version_metadata = []

    for meta in version_metadata:
        content_date_range = meta.pop("content_date_range", None)
        if content_date_range:
            meta.update({"content_start_date": content_date_range.get("start_date")})
            meta.update({"content_end_date": content_date_range.get("end_date")})
            processed_version_metadata.append(meta)

    return dataset_metadata, processed_version_metadata
