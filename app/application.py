...
class ContextualGino(Gino):
    ...
    def bind(self):
        try:
            ...
            bind = e.result()
            logging.info(f"Set bind to {bind.repr(color=True)}")
            return bind
        except LookupError:
            # not in a request
            logging.info("Not in a request, using default bind")
            return self._bind
...
async def get_engine(method: str) -> GinoEngine:
    ...
    if method in write_methods:
        logging.info("Use write engine")
        engine: GinoEngine = WRITE_ENGINE
    else:
        logging.info("Use read engine")
        engine = READ_ENGINE
    return engine
