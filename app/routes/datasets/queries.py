...
def _get_default_layer(dataset, pixel_meaning):
    ...
    if default_type == "is":
    ...
    elif default_type.endswith(AREA_DENSITY_RASTER_SUFFIX1):
        ...
        return f"{dataset}__{default_type[:len(AREA_DENSITY_RASTER_SUFFIX1)]}"
    elif default_type.endswith(AREA_DENSITY_RASTER_SUFFIX2):
        # similarly, remove _ha/yr-1 suffix for area density rasters
        return f"{dataset}__{default_type[:len(AREA_DENSITY_RASTER_SUFFIX2)]}"
    else:
...
async def _get_data_environment(grid: Grid) -> DataEnvironment:
    ...
    for row in latest_tile_sets:
        ...
        if creation_options["pixel_meaning"].endswith(AREA_DENSITY_RASTER_SUFFIX1) or creation_options["pixel_meaning"].endswith(AREA_DENSITY_RASTER_SUFFIX2):
...
def _get_area_density_layer(
    ...
    if source_layer_name.ends_with(AREA_DENSITY_RASTER_SUFFIX1):
        nm = nm.replace(AREA_DENSITY_RASTER_SUFFIX1, "")
    elif source_layer_name.ends_with(AREA_DENSITY_RASTER_SUFFIX2):
        nm = nm.replace(AREA_DENSITY_RASTER_SUFFIX2, "")
    return DerivedLayer(
        source_layer=source_layer_name,
        name=nm,
        calc="A * area",
