...
def _get_date_conf_derived_layers(
    ...
    # Date conf dates are counted as days after 2014-12-31 (so the value 1
    # represents 2015-01-01 and so on.
    decode_expression = (
        "(A.astype('timedelta64[D]') + datetime64('2014-12-31', 'D')).astype(str)"
    )
    encode_expression = (
        "(datetime64(A, 'D') - datetime64('2014-12-31', 'D')).astype(uint16)"
    )
    ...
    return [
        DerivedLayer(
            ...
            calc="(A.astype(uint16) % 10000).astype(uint16)",
            no_data=no_data_val,
